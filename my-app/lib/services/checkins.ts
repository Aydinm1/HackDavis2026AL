...
export async function getTodayScheduleAdjustments(userId: string) {
  ...
  const suggestedAdjustments = blocks.map((block) => ({
    scheduledBlockId: block.id,
    taskId: block.taskId,
    title: block.title,
    currentStartTime: block.startTime,
    currentEndTime: block.endTime,
    cognitiveLoad: block.task?.cognitiveLoad ?? null,
    suggestion:
      "Consider shortening this block, moving it later, or replacing it with a lower-load task if today's check-in is low energy or high stress.",
  }));
  ...
  return {
    ...
    summary:
      suggestedAdjustments.length > 0
        ? "High cognitive-load blocks found for today."
        : "No high cognitive-load scheduled blocks found for today.",
  };
}
